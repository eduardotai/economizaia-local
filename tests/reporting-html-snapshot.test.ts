...
const snapshotPath = path.join(process.cwd(), "docs", "snapshots", "report-html-readiness.snapshot.html");

function normalizeHtml(html: string) {
  return html.replace(/\r\n/g, "\n").trim();
}

test("snapshot do HTML do relatório com gate de prontidão permanece estável", () => {
  const html = normalizeHtml(renderReportHtml(baseReport, readinessSnapshot));
  const expected = normalizeHtml(fs.readFileSync(snapshotPath, "utf8"));

  assert.equal(html, expected);
});

test("snapshot cobre fallback conservador sem gate anexado", () => {
  const html = renderReportHtml(baseReport);

  assert.match(html, /Nenhum snapshot de prontidão foi anexado a este relatório/);
  assert.match(html, /Relatório local mock\/placeholder/);
});
